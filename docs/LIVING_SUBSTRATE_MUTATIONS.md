...
![Basic substrate grows into young, then mature substrate. Mature substrate branches into plain foundation, transport, cultivation, defense and service forms. Reinforcement is independent: no framework, bone ribs or tempered ribs, plus one optional protective lining.](diagrams/living_substrate_mutations.svg)

[Open the full-size diagram](diagrams/living_substrate_mutations.svg).

## 1. Grow the foundation

| Stage | How the player makes it | What it supports | Appearance |
|---|---|---|---|
| **Basic** | Craft dirt with Spore Culture, or apply culture to exposed eligible ground. | Culture Bowl, Digestive Sac and other starter organs. This must work before automated biomass production exists. | Sparse threads; much of the original ground remains visible. |
| **Young** | Apply bone meal or feed a deliberate biomass dose. | Established starter organs and a denser living ground cover. | Joined veins, small pores and scattered soft growth. |
| **Mature** | Feed another growth treatment to the selected young cell or marked patch. | Functional mutations and the foundations required by larger organs. | Interlocking tissue, deeper folds, varied pores and room for bushes and roots. |
...
A cell has **one structural framework** and **one optional lining**, independent of its maturity and primary function. Reinforcement can be applied to plain substrate first or to an existing functional cell later.
...
Heat lining and spatial weave are alternatives, not a linear “better than everything before it” ladder. When a build needs both services, use separate tissue routes/footings and the organ's appropriate interface parts. Do not allow every protective effect to accumulate on every floor block.

## 4. Native foundations are a separate choice

Native mutation changes the **kind of living bed**, not its reinforcement grade. These branches are not “Overworld → Nether → End” replacements for every base floor.
...
Three resulting cells might be **mature ordinary + biomass vein + bone ribs + sealed membrane**, **mature ordinary + fluid vein + tempered ribs + heat lining**, and **mature anchored + plain foundation + bone ribs + spatial weave**. Each has one clear job; none needs to lose its maturity merely to become stronger.
