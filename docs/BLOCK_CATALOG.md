...
- **Manual operation:** Use a stick to cycle the six available Bowl recipes, then insert ingredients one at a time. A water bucket deposits 1000 mB and returns its empty bucket. Empty-hand use starts the selected batch; sneak-use with an empty hand collects one occupied slot. Sneak-use a stick to cancel without consuming ingredients. Sneak-use a clock or glass bottle to spend an earned choice on incubation or water economy respectively. The status message names the selected product, stored water and completed count. All transfers pause during incubation; break and replace the Bowl to move its single saved core, contents and history.
