...
- Each helmet, leggings or boots holds 20 biomass units; the chest holds 120. **One biomass unit, BU, equals one mB of the mod's biomass fluid.**
- Repair is a mutation, not free regeneration. An unfed piece remains ordinary weak armor.
...
The initial armor does not secretly consume player health. An optional later Blood Feeder mutation explicitly enables that dangerous fallback; it starts switched off.
...
```text
Dormant flesh armor
  └─ awaken → G1 Living frame
       ├─ G2 Iron ribs ───────────┐
       └─ G2 Auric lattice ──────┤
                                ├─ G3 Diamond carapace ──┐
                                ├─ G3 Diamond tendon ───┤
                                └─ G3 Obsidian scutes ───┤
                                                        ├─ G4 Netherite lamellae
                                                        ├─ G4 Netherite mesh
                                                        └─ G4 Spatial weave

Any G2 branch can enter any G3 branch; any G3 can enter any G4 branch.
Branches at the same grade can be exchanged in a chamber.
Late Fold adaptation modifies a G4 body; it is not an unlimited fifth grade.
```

Iron and gold are alternatives. Gold supports signal handling and flexible anatomy, not stronger armor than iron. Diamond has a plated and a woven form. Obsidian is a heat/blast-oriented heavy alternative to diamond, not a compulsory ingredient in all later armor. Netherite also has rigid and flexible forms. End-grown Spatial Weave is a high-capacity alternative, not a superior carapace.
...
| G2 → G3 carapace/scutes | L6 | Nether Thermal Mantle and Thermal Nursery products | Heavy diamond or obsidian body |
| G2 → G3 tendon | L6 | Above, plus Ion Separator and precision chamber attachment | Flexible diamond body |
| G3 → G4 lamellae/mesh | L10 | Precision chamber, Thermal Mantle, real netherite supply | Chosen netherite body |
| G3 → G4 Spatial Weave | L10 | End Spatial Nursery and Spatial Conditioner | End-grown flexible body |
| Same-grade refit | Already attained that grade | Equipment and materials for the destination frame | New frame; retained identity, compatible learning and selected grafts |
...
Refitting into a lower-capacity frame preserves historical counters but clamps **active** ranks and growth points to that frame. Excess growth allocations and grafts must be selected for dormancy or removal in the preview. They do not disappear. Returning to a capable frame restores the retained ranks; the player is not asked to relearn them.
...
| Piece | Medium doses for one fusion/refit | Binder portions | Chamber biomass, G2 / G3 / G4 |
|---|---|---|---|
...
A complete first netherite-lamella set consumes eleven lamina portions: three actual netherite ingots produce twelve, leaving one portion. The earlier mineral frame is incorporated or recovered as **inert scrap**, not returned as reusable fusion doses. Same-grade refits consume the destination's full listed media; switching back and forth cannot generate materials.
...
| C-M | Metabolic work; one BU productively consumed | 100 / 1,000 / 5,000 / 20,000 / 60,000 | Work by worn mutations: movement, protection, meaningful sensing or eligible repair. A lamp counts during travel/work, not idle illumination; a scan must resolve a target. Refilling, venting, deliberate waste and Blood Feeder conversion do not count. |
...
A chamber can reallocate growth points for **100 BU per moved point plus one Binder per piece**. Reallocation does not move practice between counter categories or bypass rank requirements. It is a base refit, not a combat button.
...
Every row names a DNA source and a signature ingredient. A **complete matching genome in the DNA Bank** is required, plus physical Genetic Stock. A first install or an upgrade to rank I / II / III consumes:

- **2 / 4 / 8 matching Genetic Stock**;
- **1 / 2 / 4 of each listed signature ingredient**;
- **1 / 2 / 3 Fusion Binder portions**;
- **100 / 250 / 600 BU** of chamber biomass;
- **20 / 40 / 80 seconds** of chamber time.

These are the cost of that operation, not a refund-adjusted cumulative total. A III mutation is grown through I and II. A row with two DNA sources needs the stated stock quantity from **each**. A row marked “host” uses Spore Culture in place of Genetic Stock and needs no creature genome. Signature ingredients are consumed, including catalysts named here. Charged potion operation additionally consumes real potion doses as specified later.
...
| M4 Potion Capillary | 1 / 2 / 3 | I: G2/L2; II: G3/L6; III: G4/L10; requires Potion Infuser for every install | Witch; glass bottle | Hold 1 / 2 / 3 prepared potion doses on this piece. Dispense manually or against one configured trigger. One actual dose is consumed per activation; 5 BU and load 3 for 1 s. |
...
| H2 Lantern Gland | B; H-D | 1 / 2 / 3 | Host; glow berries | A directed biological light cone reaching 6 / 9 / 12 blocks. Illuminates nearby tunnel surfaces; never makes solid terrain see-through by itself. | 0.1 / 0.2 / 0.3 BU/s; load 1 |
...
| H5 Toxin Filter | B; H-C | 1 / 2 / 3 | Cave spider; milk bucket | Remove up to 4 / 8 / 12 s of Poison per activation, with a 20 / 15 / 10 s cooldown. Excess Poison remains. Milk's empty bucket is returned during installation. | 4 / 6 / 8 BU per activation; load 3 for 1 s |
...
| H7 Scent Pits | B; H-F | 2 / 3 / 4 | Zombie; rotten flesh | Indicate direction and rough distance band of nearby living creatures within 6 / 10 / 14 blocks. Works around thin obstacles, but gives no exact wall-through model or species-perfect ore-like outline. | 1 / 2 / 3 BU per pulse, every 4 s; load 2 |
...
| ID; branch | Schedule; counter | Slots I / II / III | DNA; signature ingredient | Effect I / II / III | Fuel and load |
|---|---|---|---|---|---|
| C1 Reservoir | B; C-M | 1 / 2 / 3 | Host; Membrane Sheet | Add 250 / 750 / 1,500 BU capacity to the chest's base 120. Capacity is not fuel generation. | No operating cost/load |
...
| C4 Impact Bladder | B; C-G | 2 / 3 / 4 | Slime; slimeball | Reduce residual physical hit damage by 10% / 15% / 20%, at most 2 / 3 / 4 health points prevented per hit. | 2 / 3 / 4 BU per health point prevented; load 4 / 6 / 8 while armed |
...
| C9 Blood Feeder | G1/L0; I only | 1 / — / — | Host; rotten flesh | Opt-in emergency conversion: 1 health point → 4 BU. At most two health points per 10 s; resulting health must remain at least half maximum. | Costs health; produces the stated BU; no load |
...
| C11 Gliding Membrane | S; C-M | 3 / 4 / 5 | Phantom; precision-conditioned Spatial Membrane | Controlled descending glide. Minimum downward speed 0.5 / 0.35 / 0.25 m/s and maximum driven horizontal speed 6 / 8 / 10 m/s in still air; cannot take off, hover or climb by repeatedly toggling. | 2 / 3 / 4 BU/s; load 5 / 6 / 7 |
...
C3 cannot make C9 profitable: healing one health point costs 10 BU while sacrificing it returns only 4. Blood Feeder injury does not count as Guarding, and repairing its self-inflicted health loss gives no Metabolic Work credit. The player must deliberately enable Blood Feeder; starvation never enables it automatically.
...
C11 needs a complete living suit for membrane control, but no specialized wing graft on the other three pieces. It replaces the chest equipment's normal role; it is not a second armor item layered onto an Elytra. Launch requires an actual ledge, jump or other legitimate source of height.
...
| ID; branch | Schedule; counter | Slots I / II / III | DNA; signature ingredient | Effect I / II / III | Fuel and load |
|---|---|---|---|---|---|
| B1 Contour Sole | B; B-F | 1 / 2 / 3 | Host; slimeball | Deliberately step onto ledges up to 1 / 1.5 / 2 blocks high when there is full body clearance. Toggle off for precise building. | 0.05 / 0.1 / 0.2 BU per assisted step; load 1 during step |
...
| B3 Landing Bladders | B; B-L | 1 / 2 / 3 | Slime; slimeball | Absorb an additional 3 / 6 / 10 blocks of fall distance beyond normal allowance. Remaining fall distance still causes its usual consequences. | 1 BU per additional block actually absorbed; load 3 for 1 s |
...
| Gliding | Complete suit + C11 | Powered control membrane; not a promise of safe landing |
...
Capacity is **430 BU**: 120 base chest + 250 reservoir + three 20-BU local reserves. Lamp and Endurance together consume 0.2 BU/s and load 2 during a run. Repair adds real cost only when there is damage to mend. The armor is still poor in a fight, but offers light, modest travel convenience and a reason to maintain the same pieces.
...
Capacity is **930 BU**. During lava immersion with Thermal Sight active, load is **8 + 2 + 2 = 12** and fuel use is **5 × 0.9 + 0.2 = 4.7 BU/s**. A full reserve lasts about **198 seconds**, without tool work or repair. Working Tendons raises that to load 15 and 5.3 BU/s. Multiple repairs may pause while the suit is working.

It is protected and capable in lava, but has no gills, underwater propulsion, Blink or Burrowing. Replacing several linings to add ocean adaptations destroys the complete thermal system. Better colony refueling matters more than another arbitrary protection multiplier.
...
The complete I burrowing system moves at **0.8 m/s**, has a **10-second stopping window**, and with Lamp II draws **24.2 BU/s at load 13**. Its 1,680 BU reserve provides about **69 seconds** of passage, or at most about **55 meters** of continuous straight travel before other costs. The player must plan a destination, retreat margin and safe cavities. This is not unlimited underground travel simply because the slots fit.

Later Spatial Weave refits provide higher caps and reduce each Burrowing graft's slot cost by one. Upgrading all four grafts to II improves speed, stability and fuel efficiency. A player who upgrades only the chest still operates a rank-I system until the other adaptations catch up.
...
The same item retains its frame, lifetime counter values, active caps, growth allocations, installed/dormant mutations, current biomass, current durability, name, dye and cosmetic trim selection. Cooldowns and environmental recovery allowances cannot be reset by briefly unequipping it, changing dimension or reconnecting.

Reinforcement is not repair. Mutation removal is not repair. A larger maximum durability preserves the previous durability percentage. A smaller reserve cannot delete or spill excess biomass silently: the chamber first transfers excess into its connected output tank and refuses the refit if there is insufficient safe capacity.

The refit preview lists the exact destination statistics, disabled allocations, incompatible grafts, remaining fuel and material cost. Confirmation is refused until there is space for every retained output. No outputs are thrown into the world because a GUI slot is full.

### Removing a mutation

An Extraction recipe in the Mutation Chamber consumes one Binder and 100 BU per removed mutation. It returns a **Recovered Graft** containing that mutation's identity and installed rank, not its owner's counters or mineral frame. Reinstalling it on the **same piece** costs 50 BU per rank and checks frame, counter, level and slot requirements again. This supports expensive but reversible specialization without making every refit require reculturing an entire genome.

Installing a Recovered Graft on a different piece requires the correct equipment slot, genome access, counter/grade prerequisites and **half the normal installation biomass and Binder cost, rounded up**. It does not consume a second set of Genetic Stock/signature ingredients already embodied in the graft. The graft is consumed exactly once. No operation creates two copies.

### Moving learning into a successor

The Memory Gland can extract a selected amount from **one named counter on one piece** into a physical Memory Sample. That amount is removed from the donor immediately. Donor ranks/levels and usable growth points are recalculated; grafts whose prerequisites are no longer met become dormant.

The recipient must be the same equipment slot and the same counter type: boot Ascent cannot become helmet Field Study, and chest Metabolic Work cannot become leggings Burrowing. Transfer fills only up to the recipient's current counter ceiling; any remainder stays in the physical sample. It does not bypass fusion prerequisites that depend on the recipient's resulting level or colony equipment.

A DNA Bank may remember the recipe and that a sample exists. It is not a backup from which erased physical experience can be copied. Death can therefore lose a valuable lineage unless the player previously moved some learning into a stored sample—and accepted the weaker donor that resulted.

### Refitting a rigid veteran into a flexible specialist

An L15 Diamond Carapace helmet has all five counters at R3, filling that frame's ceiling. A Diamond Tendon refit retains those same values and L15; it does **not** jump to L20. The new body permits R4, allowing five more levels through further use. It also trades protection 3 → 2 and durability 330 → 220 for anatomy 8 → 11.

Conversely, an L20 Diamond Tendon helmet refitted into carapace retains its history but operates at most L15/R3. The player must pick which of its growth allocations and oversized grafts become dormant. The preview must make that cost impossible to miss.
...
- Spatial Weave competes with netherite protection using fourteen slots and R5 ceilings.
- Blink is a boot specialization, not a free universal suit bonus for defeating the dragon.
- Gliding occupies chest anatomy and excludes the Burrow Mantle. The player chooses an aerial traveler or an underground traveler, not a switch that does both without a refit.
- Producing the needed conditioned membranes requires the End-grown substrate/conditioning infrastructure. Stockpiling pearls in the Overworld does not replace it.

### Fold adaptation: reserved long-term branch

After establishing a Fold colony and obtaining Adaptive Gel, a G4 piece can receive an **Adaptive Interface**. Proposed installation: two Adaptive Gel, one Thermal Lining, one precision-conditioned Spatial Membrane, four Binder and 1,000 BU. It occupies **two anatomy slots permanently while installed** and does not increase protection, counter ceilings or level.

The interface stores two configurations of that piece's **already-owned** grafts. Each complete configuration, including the two-slot interface, must fit independently. Inactive grafts are physically retained inside the interface and cannot simultaneously exist as external Recovered Grafts. Switching takes 30 seconds at a powered service pedestal, costs 200 BU per switched piece and requires no hostile damage during the operation. Stored biomass capacity cannot be temporarily doubled by switching profiles.

Both configurations must be legal: an interface may switch a helmet between thermal and aquatic vision packages, but cannot run both linings together. Coordinated suit abilities are recalculated from what is actually active on all four pieces. Switching one part may deliberately leave the full system incomplete until the rest are serviced.

The proposed Manyfold boss can eventually unlock a **Reciprocal Graft** permitting one preselected profile change in the field: 10 seconds stationary, 100 BU per changed piece and a five-minute shared cooldown. The normal two-slot tax, mutually exclusive profiles and interruption by hostile damage remain. This is a deployment convenience earned after the boss, not extra active anatomy or invulnerability during transformation. Its exact boss material recipe remains reserved for the Fold progression document; it is not an available G4 crafting shortcut.
...
Exact accessory recipes and capacities belong in their own item catalog. Armor must remain usable without requiring every Curios slot to be filled.
...
- Refueling and refitting use bounded tanks/inventories with back-pressure. No armor operation vents excess fluid or throws items into the world as its automatic overflow behavior.
