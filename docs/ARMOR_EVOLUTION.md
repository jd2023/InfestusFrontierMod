...
Current rules below supersede the former rank-sum/growth-point model.
[Open questions](OPEN_QUESTIONS.md) owns every unresolved parameter. Exact retained
frame/mutation numbers are labeled **draft candidates**, not approved balance.
[Items](ITEM_CATALOG.md) owns preparation recipes; [Blocks](BLOCK_CATALOG.md) owns
service structures. This file owns the state and behavior of an armor piece.
...
| Tissue durability | Condition healed with fuel/services; zero means collapsed equipment under the retained draft. |
...
## 2. Start, feed, practice, fuse

1. Craft a dormant piece using the familiar armor silhouette and rotten flesh.
2. Awaken that same piece in an Awakening Cradle. Its first reserve is empty.
3. Fill it at a Biomass Bladder before wearing it; later use Papilla or portable supply.
4. Eligible activities teach the piece automatically within its shared capacity.
5. Prepare bioactive fusion media and fuse a legal child frame. Capacity rises;
   counters keep their existing values. Fusion grants no retrospective practice.
6. Install compatible prepared DNA grafts; continue forward within their branches.
7. Reduce an unwanted counter through a paid ritual if capacity must be freed.
   This changes learning, not the material or mutation branch.

Recipes belong to Items; operating bodies to Blocks. Starting statistics,
fusion costs and activity conversions are Q-001/Q-002/Q-005.

## 3. Shared capacity and partial counter reduction
...
Example only: capacity 100, sprint 80 and swimming 20 means learning stops.
Fusion to capacity 160 retains 80 and 20, opening 60 points. These are explanatory
values, not the chosen capacity schedule.
...
- Proposed consequence: recompute learned buffs immediately; falling below a threshold
  loses that threshold's buff. Whether an installed mutation becomes inactive when
  a prerequisite is reduced needs Q-003; its anatomy cannot be refunded or exchanged.
- Fixed amount, ingredients, energy, duration, minimum-count behavior and exact
  structure are Q-003. Ten points was an example.
- No point-transfer/storage recipe is currently approved. Memory Gland/Sample
  candidates are held behind Q-003, not a way around paid reduction.

## 4. Activity ownership and buffs

This is a **candidate activity set**, awaiting Q-002. IDs identify the owning piece,
not an approved points-per-event schedule. Distinct physical events can occur
together, but the same activity cannot credit a second piece. For example,
swimming distance belongs only to leggings; helmet air use is a separate breath
event, not a second swimming-distance award. Burning and hot-floor contact must
be classified rather than paying both counters for the same damage event.
...
No old R0–R5 conversion table remains: meter, second and damage counts need
normalization before sharing a capacity. No buff numbers can be inferred from
the illustrative cap example. Q-002 must give every counter its exact credit
formula, buff thresholds and combined ceilings before implementation.

Turning on a mutation supplies anatomy; practice improves its use. Swimming
alone does not grow gills and gliding does not create an Elytra. Whether a practice
threshold gates installation is Q-006, not the former automatic rank schedule.

### Death rescue

The owner wants death learning to grant a totem-like rescue usable once, with a
bed or special reset interaction. Learning, charge, rescue effects and reset
semantics are **not yet distinguishable from the request**; resolve Q-004 before
implementing any of them. Do not silently make sleep erase the shared-capacity
death counter, recharge rescue or preserve both. A rescue is not automatically
a recorded actual death. Chest ownership above is a proposal.
...
### Retained frame-stat candidates

These numbers are **draft candidates**, pending Q-001/Q-005. Protection and durability
values are helmet / chest / leggings / boots. The old per-counter ceilings are withdrawn.
...
Retained frame-effect candidates: Obsidian Scutes reduce blast knockback
...
Neither effect grants lava or fire immunity. Q-005 ratifies these modifiers.

The frame tree and statistics remain candidates under Q-005; amethyst has no
assigned parent yet. G is the proposed fusion level, not an extra grade plus
an activity-derived item level. Numeric shared capacities are Q-001.

Iron emphasizes structural plating; Auric Lattice emphasizes anatomical space.
Forward fusion retains installed compatible anatomy and history. No sibling
exchange, backward fusion or reconverging material branch.

### Turning minerals into living fusion materials

Use Mineral Gizzard and Washing Kidney to prepare real mineral, then the listed
culture/thermal/precision process to create the target medium. See
[Item Catalog](ITEM_CATALOG.md) for all quantities, times and returned wash fluid.

Ferrocyte Paste → Iron Ribs; Auric Myelin → Auric Lattice; Faceted Chitin →
Diamond Carapace; Vitreous Scute → Obsidian Scutes; Diamond-Fiber Matrix →
Diamond Tendon; the corresponding prepared netherite/spatial media → their
named children. Medium preparation never multiplies its mineral input.

Q-005 owns fusion installation dose/time/energy and chamber attachment requirements.
The former L2/L6/L10 practice-sum fusion prerequisites are withdrawn. The owner's
exponentially harder fusion requires a defined cost schedule, not an invented
new personal XP requirement. Minecraft XP policy is Q-008.

## 6. Mutation installation and reading the candidate tables
...
Tables retain concrete effect, slot and fuel proposals so they can be compared.
All old B/A/S schedules, practice ranks and L-level gates are withdrawn.
**Q-006** supplies the replacement fusion-level, DNA-combination and any learning
prerequisites. These rows are not implementation-ready until those fields are resolved.
...
Elytra once. Q-006 must make automatic player feeding and powered flight later
progression, not the former generic Basic access schedule.
...
|---|---|---|---|---|
| M1 Healing Membrane | 1 / 1 / 2 | Q-006 | Host; Membrane Sheet | Restore 1 / 3 / 6 durability every 10 s to this piece. Costs 2 BU per durability actually restored; reserves load 1 / 2 / 4 while healing. Pauses during starvation; does not restore player health. |
| M2 Thermal Lining | 2 / 3 / 4 | Q-006 | Magma cube; Thermal Lining | One quarter of a sealed thermal suit. Full-set effects are below; individual incomplete linings confer no player fire immunity. |
| M3 Pelagic Lining | 2 / 3 / 4 | Q-006 | Turtle; Membrane Sheet | One quarter of a coordinated aquatic suit. Enables the underwater handling and efficiency benefits below, not free breathing by itself. |
| M4 Potion Capillary | 1 / 2 / 3 | Q-006 | Witch; Potion Vesicle | Hold 1 / 2 / 3 prepared potion doses on this piece. Dispense manually or against one configured trigger. One actual dose is consumed per activation; 5 BU and load 3 for 1 s. |
...
| ID; branch | Access decision | Slots I / II / III | DNA; signature ingredient | Effect I / II / III | Fuel and load |
|---|---|---|---|---|---|
| H1 Field Lens | Q-006 | 1 / 2 / 3 | Cow; Membrane Sheet | Examine a visible target from 12 / 20 / 28 blocks; show its species, sampling coverage and currently observable condition. Does not disclose unseen inventory or an entire mod's private mob state. | 1 BU per examination; load 1 for its 1 s focus |
| H2 Lantern Gland | Q-006 | 1 / 2 / 3 | Host; Lumen Secretion | A directed biological light cone reaching 6 / 9 / 12 blocks. Illuminates nearby tunnel surfaces; never makes solid terrain see-through by itself. | 0.1 / 0.2 / 0.3 BU/s; load 1 |
| H3 Nocturnal Membrane | Q-006 | 1 / 2 / 3 | Bat; Membrane Sheet | Low-light vision within 8 / 12 / 16 blocks. Preserves silhouette and movement visibility, with reduced color information. Does not light the area for other players. | 0.1 / 0.15 / 0.2 BU/s; load 1 / 1 / 2 |
| H4 Aquatic Eyes | Q-006 | 1 / 2 / 3 | Squid; Membrane Sheet | Underwater clarity target of 8 / 16 / 24 blocks, limited by actual terrain visibility. No seeing ores through walls. | 0.2 / 0.4 / 0.6 BU/s; load 1 / 2 / 2 |
| H5 Toxin Filter | Q-006 | 1 / 2 / 3 | Cave spider; Antitoxin Serum | Remove up to 4 / 8 / 12 s of Poison per activation, with a 20 / 15 / 10 s cooldown. Excess Poison remains. Milk containers return during serum preparation. | 4 / 6 / 8 BU per activation; load 3 for 1 s |
| H6 Thermal Sight | Q-006 | 2 / 3 / 4 | Blaze; Thermal Lining | Distinguish exposed hot targets and hot surfaces through smoke/fire overlays within 12 / 20 / 28 blocks. Cannot look through opaque rock. | 0.2 / 0.3 / 0.5 BU/s; load 2 |
| H7 Scent Pits | Q-006 | 2 / 3 / 4 | Zombie; Scent Concentrate | Indicate direction and rough distance band of nearby living creatures within 6 / 10 / 14 blocks. Works around thin obstacles, but gives no exact wall-through model or species-perfect ore-like outline. | 1 / 2 / 3 BU per pulse, every 4 s; load 2 |
| H8 Stone Sense | Q-006 | 3 / 4 / 5 | Enderman; Diamond-Fiber Matrix | Burrowing adaptation: distinguish passable rock and nearby air pockets within 3 / 5 / 7 blocks while inside ground. Reveals cavities, not ore identities or block inventories. | Included in the coordinated Burrowing cost; alone, 1 BU per deliberate pulse, load 2 |
| H9 Wither Sieve | Q-006 | 2 / 3 / 4 | Wither skeleton; Thermal Lining | Remove up to 4 / 8 / 12 s of Wither, cooldown 20 s. Does not prevent the attack, cancel boss damage or permanently immunize the wearer. | 10 / 15 / 20 BU per activation; load 5 for 1 s |
| H10 Colony Reader | Q-006 | 1 / 2 / 3 | Bee; Auric Myelin | Show selected organ status and recent flow, and issue an already-authorized helper order, within 12 / 20 / 28 blocks. Higher rank improves reach, not drone population. | 1 BU per query/order; load 1 for 1 s |

H2 and H3 can coexist. H3, H4 and H6 are installed anatomy, not exchangeable branches; only one visual display mode is active at a time. H7 uses a directional cue. H10 cannot read unloaded chunks or bypass ownership. New counter-buff interactions and reach ceilings are Q-002/Q-009.
...
| ID; branch | Access decision | Slots I / II / III | DNA; signature ingredient | Effect I / II / III | Fuel and load |
|---|---|---|---|---|---|
| C1 Reservoir | Q-006 | 1 / 2 / 3 | Host; Membrane Sheet | Add 100 / 600 / 1,500 BU capacity to the chest's base 40. Capacity is not fuel generation. | No operating cost/load |
| C2 Pump Heart | Q-006 | 1 / 2 / 3 | Host; Auric Myelin | Add 4 / 7 / 10 to complete-suit output. Does not multiply reserve capacity. | 0.1 / 0.2 / 0.4 BU/s only while demand exceeds the unmodified output; no additional load |
| C3 Regrowth Lobe | Q-006 | 2 / 3 / 4 | Axolotl; Membrane Sheet | Restore 1 health point every 10 / 6 / 4 s, starting 5 s after the last hostile hit. Healing stops at full health and is not damage immunity. | 10 BU per health point; load 4 while healing |
| C4 Impact Bladder | Q-006 | 2 / 3 / 4 | Slime; Elastic Gel | Reduce residual physical hit damage by 10% / 15% / 20%, at most 2 / 3 / 4 health points prevented per hit. | 2 / 3 / 4 BU per health point prevented; load 4 / 6 / 8 while armed |
| C5 Thermal Exchange | Q-006 | 2 / 3 / 4 | Magma cube; Thermal Lining | Reduce Thermal Mode biomass cost by 10% / 20% / 30%. No conversion of environmental heat into free suit biomass. | No additional BU cost; load 2 / 3 / 4 while exchanging |
| C6 Gill Bellows | Q-006 | 2 / 3 / 4 | Turtle; Membrane Sheet | Supply up to 10 / 30 / 60 s of breathing per pulse. Refills only missing air; unused supply is not banked beyond the current air capacity. | Full pulse costs 3 / 6 / 10 BU, prorated for missing air; load 2 / 3 / 4 while operating |
| C7 Blast Baffles | Q-006 | 2 / 3 / 4 | Creeper; Vitreous Scute | Reduce residual explosion damage by 10% / 15% / 20%, at most 3 / 5 / 7 health points per explosion. | 3 BU per health point prevented; load 4 / 6 / 8 while armed |
| C8 Burrow Mantle | Q-006 | 3 / 4 / 5 | Enderman; Diamond-Fiber Matrix | Whole-body passage support. Together with H8/L7/B8 gives a 10 / 20 / 35 s safe stopping window. | Whole Burrowing mode: 24 / 20 / 18 BU/s; load 12 / 10 / 8 |
| C9 Feeding Lobe | Q-006: late access | 1 / 2 / 3 | Host; Nutrient Mash | Automatically consume permitted real food from 1 / 2 / 3 selected inventory slots when hunger is at or below 14 / 16 / 18 points. At most one food per 10 s; normal food effects apply. | 1 BU per food; load 1 for 1 s. No biomass-to-food conversion. |
| C10 Service Tendril | Q-006 | 1 / 2 / 3 | Bee; Membrane Sheet | Transfer stored biomass at 2 / 4 / 6 BU/s to one explicitly selected compatible tool or helper within 2 blocks. Stops when its tank is full. | Actual transferred BU; load 2; transfers give no activity credit |
| C11 Elytral Wings | Q-006 | 3 / 4 / 5 | Phantom; precision-conditioned Spatial Membrane; actual Elytra at I only | I: controlled descending glide. II: powered forward flight and climbing. III: controlled takeoff and hover. Driven speed ceiling 6 / 8 / 10 m/s. | 2 / 6 / 10 BU/s; load 5 / 8 / 12 |
| C12 Nutrient Intake | Q-006 | 1 / 2 / 3 | Host; Capillary Gel | Refill from 1 / 2 / 3 selected sealed biomass containers at up to 2 / 5 / 10 BU/s, stopping at capacity and retaining partial containers. | Transfers actual biomass; no output increase or practice credit. |
| C13 Digestive Crop | Q-006 | 2 / 3 / 4 | Zombie; Digestive Enzyme | Automatically digest permitted biological materials from 1 / 2 / 3 selected inventory slots into chest biomass. Proposed net yield: 60% / 70% / 80% of the material's declared basic stationary digestion reference; delivery capped at 1 / 2 / 4 BU/s. | Load 1 / 2 / 2 while digesting; no starting biomass required. Consumes real feed, not player health or hunger. Conversion itself grants no practice. |
...
Container intake trades expedition packing for predictable transfer with no conversion loss. Digestion uses found/farmed feed but takes more anatomy and time, returns less biomass per material than base processing, and may compete for metabolic output. At the retained draft rates, C13's maximum 4 BU/s is below C8's minimum 18 BU/s; carried feed alone cannot sustain Burrowing indefinitely. Manual and tissue refueling remain available on either specialization. Exact feed yields and the proposed rates require balance tests before implementation.
...
| ID; branch | Access decision | Slots I / II / III | DNA; signature ingredient | Effect I / II / III | Fuel and load |
|---|---|---|---|---|---|
| L1 Running Tendons | Q-006 | 1 / 2 / 3 | Rabbit; Membrane Sheet | +10% / 20% / 30% grounded sprint speed. | 0.5 / 1 / 2 BU/s while sprinting; load 1 / 2 / 3 |
| L2 Endurance Mesh | Q-006 | 1 / 2 / 3 | Host; Membrane Sheet | Reduce sprint-related hunger expenditure by 15% / 30% / 45%; does not reduce starvation damage or replace food. | 0.1 / 0.2 / 0.4 BU/s while sprinting; load 1 |
| L3 Swimming Muscles | Q-006 | 1 / 2 / 3 | Dolphin; Membrane Sheet | +15% / 30% / 45% self-propelled swimming speed. | 0.4 / 0.8 / 1.2 BU/s; load 2 / 3 / 4 |
| L4 Bracing Tendons | Q-006 | 1 / 2 / 3 | Turtle; Bone Plate | While deliberately braced on a surface and using a tool, reduce hostile knockback by 20% / 40% / 60%. Walking releases the brace. | 0.2 / 0.4 / 0.6 BU/s while braced; load 1 / 2 / 3 |
| L5 Working Tendons | Q-006 | 2 / 3 / 4 | Zombie; Bone Plate | +10% / 20% / 30% manual tool working speed. Does not change harvest tier, loot, ore identity, durability cost per action or machine speed. | 0.3 / 0.6 / 1 BU/s during actual tool work; load 2 / 3 / 4 |
| L6 Stalking Fibers | Q-006 | 1 / 2 / 3 | Fox; Membrane Sheet | +20% / 40% / 60% crouched movement speed. Does not make the wearer invisible or silence every action. | 0.1 / 0.2 / 0.3 BU/s while moving crouched; load 1 |
| L7 Burrowing Muscles | Q-006 | 3 / 4 / 5 | Enderman; Diamond-Fiber Matrix | Coordinated Burrowing travel speed 0.8 / 1.2 / 1.6 m/s through eligible terrain, including deliberate upward/downward movement. | Included in C8 cost/load |
| L8 Fast-Lane Tendons | Q-006 | 2 / 3 / 4 | Host; Auric Myelin | +40% / 70% / 100% grounded travel speed on a compatible mutated fast-lane tissue. Needs a complete living suit and Surface Key II or III. | 0.5 / 1 / 2 BU/s on the active lane; load 2 / 3 / 4 |
...
| ID; branch | Access decision | Slots I / II / III | DNA; signature ingredient | Effect I / II / III | Fuel and load |
|---|---|---|---|---|---|
| B1 Contour Sole | Q-006 | 1 / 2 / 3 | Host; Elastic Gel | Deliberately step onto ledges up to 1 / 1.5 / 2 blocks high when there is full body clearance. Toggle off for precise building. | 0.05 / 0.1 / 0.2 BU per assisted step; load 1 during step |
| B2 Climbing Hooks | Q-006 | 2 / 3 / 4 | Spider; Bone Plate | Climb a contacted wall at 0.8 / 1.2 / 1.6 m/s. No climbing air, remote walls or unlimited hanging without fuel. | 0.4 / 0.8 / 1.2 BU/s; load 2 / 3 / 4 |
| B3 Landing Bladders | Q-006 | 1 / 2 / 3 | Slime; Elastic Gel | Absorb an additional 3 / 6 / 10 blocks of fall distance beyond normal allowance. Remaining fall distance still causes its usual consequences. | 1 BU per additional block actually absorbed; load 3 for 1 s |
| B4 Propulsive Fins | Q-006 | 2 / 3 / 4 | Squid; Membrane Sheet | +20% / 35% / 50% swimming speed. | 0.3 / 0.6 / 1 BU/s while swimming; load 2 / 3 / 4 |
| B5 Spring Heel | Q-006 | 2 / 3 / 4 | Rabbit; Bone Plate | Charged ground jump reaching 1.5 / 2.5 / 3.5 blocks above takeoff. Requires surface contact; not a midair second jump. | 3 / 6 / 10 BU; load 3 for 1 s; cooldown 6 s |
| B6 Blink Tendon | Q-006 | 3 / 4 / 5 | Enderman; precision-conditioned Spatial Membrane | Relocate up to 4 / 7 / 10 blocks to a visible clear supported destination. Does not pass through opaque terrain. | 20 / 30 / 45 BU; load 6 for 1 s; cooldown 12 / 10 / 8 s |
| B7 Surface Key | Q-006 | 1 / 2 / 3 | Host; Spore Culture | Store 1 / 2 / 4 consented colony tissue signatures. I identifies wearer to compatible defensive tissue; II also enables Fast-Lane Tendons; III adds capacity for multiple colonies, not immunity to every hostile biomass fluid. | No operating cost/load |
| B8 Ground Anchor | Q-006 | 3 / 4 / 5 | Enderman; Diamond-Fiber Matrix | Coordinate body contact for Burrowing in all directions. Effective rank limits the complete burrowing system. Does not supply air or light. | Included in C8 cost/load |
...
Retained starting-capacity candidate: 10 BU each for helmet, leggings and boots;
40 BU for chest, before a Reservoir graft. Capacity contains no free fuel.
Q-009 must ratify reserves and their fusion progression.
...
Adaptation ownership, whether its learning shares the activity capacity, damage
cadence, fatality, difficulty and death behavior are Q-007. The old extra maturation
counter/track model is not an approved exception to the shared cap. No numeric
damage or adaptation threshold is selected here.
...
## 12. Additional DNA combinations under review

These alternatives are explicit **proposals** for Q-006; they do not silently
replace the retained single-source recipes above. They are authored game biology,
not assertions about vanilla abilities.

| Ability | Owning piece | Candidate DNA combination | Required distinction |
|---|---|---|---|
| Automatic feeding | Chest | Cow + pig | Actual food to player hunger, not armor biomass |
| Player healing | Chest | Axolotl + host sample | Pays for health restored; no invulnerability |
| Berserk | Chest | Hoglin + wolf | Offensive burst with defensive cost and recovery; values Q-006 |
| Powered flight | Chest | Phantom + bee + actual Elytra ingredient | Permanent aerial branch, not Burrowing branch exchange |
| Water breathing | Chest Gill Bellows in retained anatomy | Cod + turtle | Learning ownership may be helmet; anatomy does not duplicate counters |
| Night vision | Helmet | Cat + glow squid | Personal vision, not world illumination |
| Burrowing | Coordinated H8/C8/L7/B8 | Silverfish + enderman | Ground passage, fuel, stability, air and visibility |
| High jump | Boots | Rabbit + goat | Does not itself protect landing |
| Soft landing | Boots | Slime + chicken | Limited fall absorption |
| Heat protection | Thermal anatomy | Strider + magma cube | Heat specialty, not all damage |
| Wall climbing | Boots | Spider + silverfish | Surface contact required |
| Aquatic propulsion | Leggings | Dolphin + salmon | Water movement, not breathing |

Berserk needs a catalog/guide ID after its recipe and behavior are selected.
Death rescue is the learning branch in section 4, not a guessed DNA graft.

## 13. Persistence, display and compatibility
...
Their preparation belongs to Items; access/cost approval is Q-006/Q-031.

## 14. Safety and implementation boundaries
...
Retained proposed limits for expensive visuals: Burrowing checks a 3×3×3 body
...
inventory pass per wearer per second in the retained draft. Digestion saves one
