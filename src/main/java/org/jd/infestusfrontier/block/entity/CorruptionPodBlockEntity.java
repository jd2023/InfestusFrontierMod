...
public class CorruptionPodBlockEntity extends BlockEntity implements MenuProvider {
    private static final Logger LOGGER = LogUtils.getLogger();
    private Infester infester;


    public CorruptionPodBlockEntity(BlockPos pos, BlockState state) {
        super(InfestusBlockEntities.CORRUPTION_POD_ENTITY.get(), pos, state);
    ...
    public static <T extends BlockEntity> void tick(Level level, BlockPos pos, BlockState state, T be) {
        ...
        if (level instanceof ServerLevel serverLevel) {
            if (be instanceof CorruptionPodBlockEntity entity) {
