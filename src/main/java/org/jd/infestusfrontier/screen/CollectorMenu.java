...
public class CollectorMenu extends AbstractContainerMenu {
    ...
    public CollectorMenu(int containerID, Inventory inv, BlockEntity entity, ContainerData data){
        super(InfestusMenuTypes.COLLECTOR_MENU.get(), containerID);
        checkContainerSize(inv, 8);

        blockEntity = ((CollectorBlockEntity) entity);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            this.addSlot(new SlotItemHandler(iItemHandler, 0, 80, 50));
        });
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, InfestusBlocks.COLLECTOR.get());
    }
}
