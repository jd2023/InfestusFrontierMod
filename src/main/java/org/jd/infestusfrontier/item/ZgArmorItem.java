...
import org.jd.infestusfrontier.item.ZgArmorMaterials;
...
public class ZgArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, MobEffectInstance> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(ZgArmorMaterials.FLESH_TIER_ONE,
                            new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 10, 0)).build();

    public ZgArmorItem(ArmorMaterial material, EquipmentSlot slot, Properties settings) {
    ...
    public void onArmorTick(ItemStack stack, Level world, Player player) {
        if(!world.isClientSide()) {
            if(hasFullSuitOfArmorOn(player) && isOnInfestusNetwork(player)) {
