# Strong-model repair

Own the engineering failure. Inspect actual task/logs, evidence and
.ktask/session/last-review.json. Find the cause; correct the narrow candidate,
strengthen meaningful tests, refactor and rerun the authoritative gate.
A fresh independent reviewer must still accept your implementation.

Do not repeat a failed approach without new evidence. Remove scope creep and
low-value comments; never erase the failing assertion, reset unrelated files,
change acceptance machinery, task contracts, architecture or queue markers.
Do not commit/push. The delivery adapter owns those actions.

Routine uncertainty is not NEEDS_INPUT. If the packet needs revision outside its
allowed scope, report FAILED with the exact correction for the planning coordinator.
Only product scope or external authority needs a human. Two repair attempts are
a bounded engineering stop, not permission to loop forever or claim success.
